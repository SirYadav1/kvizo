...
fun UpdaterScreen(vm: AppViewModel, nav: NavHostController) {
    ...
    Column(modifier = Modifier.fillMaxSize().statusBarsPadding().navigationBarsPadding().verticalScroll(rememberScrollState()).padding(horizontal = 16.dp)) {
        ...
        ForgeCard(modifier = Modifier.fillMaxWidth().padding(bottom = 14.dp)) {
            Row(modifier = Modifier.padding(16.dp), verticalAlignment = Alignment.CenterVertically) {
                ...
                Column(modifier = Modifier.padding(start = 14.dp).weight(1f)) {
                    Text("Kvizo ${BuildConfig.VERSION_NAME}", fontFamily = SpaceGrotesk, fontWeight = FontWeight.Bold, fontSize = 16.sp)
                ...
                Row(modifier = Modifier.fillMaxWidth().padding(vertical = 10.dp), verticalAlignment = Alignment.CenterVertically) {
                    ...
                    Row(verticalAlignment = Alignment.CenterVertically, modifier = Modifier.fillMaxWidth().padding(vertical = 6.dp)) {
                        Column(Modifier.weight(1f)) {
                            ...
                            Text("Community announcements from Kvizo", fontSize = 11.5.sp, color = MaterialTheme.colorScheme.onSurfaceVariant)
                        }
                        Switch(checked = settings.announcementsEnabled, onCheckedChange = { vm.setAnnouncementsEnabled(it) })
                    }
                }
            }
        }
        ...
        if (state == "available") {
            OutlinedButton(
                onClick = {
                    val intent = android.content.Intent(android.content.Intent.ACTION_VIEW, android.net.Uri.parse(updateUrl.ifBlank { "https://github.com/SirYadav1/kvizo/releases" }))
    ...
    Column(
        ...
        ForgeCard(modifier = Modifier.fillMaxWidth().padding(bottom = 14.dp)) {
            Column(modifier = Modifier.fillMaxWidth().padding(20.dp), horizontalAlignment = Alignment.CenterHorizontally) {
                Image(
                    painter = painterResource(com.quizforge.app.R.drawable.ic_kvizo_logo),
                    contentDescription = "Kvizo logo",
                    modifier = Modifier.size(72.dp)
                )
                Text("Kvizo", fontFamily = SpaceGrotesk, fontWeight = FontWeight.Bold, fontSize = 20.sp, modifier = Modifier.padding(top = 10.dp))
        ...
        ForgeCard(modifier = Modifier.fillMaxWidth().padding(bottom = 14.dp)) {
            Column(modifier = Modifier.padding(16.dp)) {
                Row(verticalAlignment = Alignment.CenterVertically) {
                    Image(
                        painter = painterResource(com.quizforge.app.R.drawable.ic_dev_avatar),
...
private fun Modifier.rotate(deg: Float): Modifier = this.graphicsLayer { rotationZ = deg }
