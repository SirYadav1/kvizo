...
class CommunityQuizManager(private val context: Context) {
    ...
    suspend fun fetchAndImport(): Result<List<CommunityQuiz>> = withContext(Dispatchers.IO) {
        try {
            ...
            // .sig file is base64 text — decode before verifying
            val sigRaw = fetchUrlBytes(CommunityKeys.COMMUNITY_SIG_URL)
            val sig = try {
                Base64.getDecoder().decode(String(sigRaw).trim())
            } catch (_: IllegalArgumentException) {
                sigRaw // already raw bytes
            }
    ...
    private fun verifyEd25519Signature(
        ...
        return try {
            ...
            // BouncyCastle lightweight API — works on ALL Android versions (minSdk 26),
            // no JCA provider/platform-version branching needed.
            val verifier = org.bouncycastle.crypto.signers.Ed25519Signer()
            verifier.init(false, org.bouncycastle.crypto.params.Ed25519PublicKeyParameters(pubBytes, 0))
