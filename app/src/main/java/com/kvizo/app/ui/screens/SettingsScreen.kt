...
fun SettingsScreen(vm: AppViewModel, nav: NavHostController) {
    ...
    Column(modifier = Modifier.fillMaxSize().statusBarsPadding().navigationBarsPadding().verticalScroll(rememberScrollState()).padding(horizontal = 16.dp)) {
        Text("Settings", fontFamily = com.kvizo.app.ui.theme.SpaceGrotesk, fontWeight = FontWeight.Bold, fontSize = 24.sp, letterSpacing = (-0.3).sp, modifier = Modifier.padding(top = 12.dp, bottom = 10.dp))
        ...
        // ---------- App info ----------
        SectionTitle("App")
        SettingsRow(Icons.Filled.SystemUpdate, "Updater", "Check for updates & notifications", com.kvizo.app.ui.theme.Violet) { nav.navigate(Routes.UPDATER) }
        SettingsRow(Icons.Filled.History, "Changelog", "What's new in every build", Orange) { nav.navigate(Routes.CHANGELOG) }
        SettingsRow(Icons.Filled.Info, "About", "Version, developer & contributors", com.kvizo.app.ui.theme.Green) { nav.navigate(Routes.ABOUT) }
