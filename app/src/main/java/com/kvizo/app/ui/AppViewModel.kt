...
import com.kvizo.app.data.AttemptResult
import com.kvizo.app.data.Badge
import com.kvizo.app.data.LeaderboardEntry
...
class AppViewModel(app: Application) : AndroidViewModel(app) {
    ...
    private suspend fun syncRemoteQuizzesNow() {
        ...
        try {
            ...
            // offline — cache fallback keeps existing data
        }
    }
    ...
    fun importCommunityQuiz(quiz: CommunityQuiz, onDone: (String) -> Unit = {}) {
        viewModelScope.launch {
            val pid = profile?.id
    ...
    fun importSharedQuiz(shared: com.kvizo.app.util.ShareCodec.SharedQuiz): Quiz {
        val pid = profile?.id ?: throw IllegalStateException("No active profile")
        val now = System.currentTimeMillis()
        val quiz = Quiz(
            id = UUID.randomUUID().toString(),
    ...
    private fun ensureSoundPool(): SoundPool? {
        ...
        return try {
            ...
            soundSplash = sp.load(appContext, R.raw.sound_splash, 1)
            soundPool = sp
            sp
        } catch (_: Exception) {
            null
    ...
    /** Soft pop + chime intro jingle for the splash animation (gentle volume). */
    ...
    suspend fun latestRelease(): Pair<String, String>? = withContext(Dispatchers.IO) {
        try {
            val conn = URL("https://api.github.com/repos/SirYadav1/kvizo/releases/latest").openConnection() as java.net.HttpURLConnection
