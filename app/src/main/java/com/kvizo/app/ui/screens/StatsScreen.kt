...
import com.kvizo.app.ui.components.DonutChart
import com.kvizo.app.ui.components.ForgeCard
import com.kvizo.app.ui.components.HeatmapCalendar
import com.kvizo.app.ui.components.HBarChart
import com.kvizo.app.ui.components.LineChart
import com.kvizo.app.ui.components.SectionTitle
import com.kvizo.app.ui.components.StatCard
...
fun StatsScreen(vm: AppViewModel, nav: NavHostController) {
    ...
    val availableDates = remember(attempts) {
        attempts.map { com.kvizo.app.logic.XpEngine.dateStr(it.attemptedAt) }.distinct().sortedDescending()
    }
    ...
    val dayAttempts = remember(attempts, effectiveDate) {
        attempts.filter { com.kvizo.app.logic.XpEngine.dateStr(it.attemptedAt) == effectiveDate }
    }
    ...
    val daySet = filtered.map { com.kvizo.app.logic.XpEngine.dateStr(it.attemptedAt) }.toSet()
    val streak = com.kvizo.app.logic.XpEngine.currentStreak(daySet)
    ...
    Column(modifier = Modifier.fillMaxSize().statusBarsPadding().navigationBarsPadding().padding(horizontal = 16.dp)) {
        Row(modifier = Modifier.fillMaxWidth().padding(top = 12.dp), verticalAlignment = Alignment.CenterVertically) {
            Text("Statistics", fontFamily = com.kvizo.app.ui.theme.SpaceGrotesk, fontWeight = FontWeight.Bold, fontSize = 22.sp, modifier = Modifier.weight(1f))
                ...
                listOf("All Time", "This Week", "This Month").forEach { r ->
                    ...
                    Box(
                        modifier = Modifier
                            .weight(1f)
                            .background(
                                if (selected) com.kvizo.app.ui.theme.violetGradient()
                                ...
                                Box {
                                    ...
                                    DropdownMenu(expanded = dateMenu, onDismissRequest = { dateMenu = false }) {
                                        availableDates.forEach { d ->
                                            val count = attempts.count { com.kvizo.app.logic.XpEngine.dateStr(it.attemptedAt) == d }
...
private fun buildHeatmap(attempts: List<Attempt>): Map<String, Int> {
    ...
    attempts.forEach { a ->
        val day = com.kvizo.app.logic.XpEngine.dateStr(a.attemptedAt)
        map[day] = (map[day] ?: 0) + 1
    }
    return map
}
